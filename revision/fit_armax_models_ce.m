...
if pre_or_post == "post"
    ...
    savingFileErr = flag.post_err_model_output;
    savingFileCorr = flag.post_corr_model_output;
    load(flag.post_saved_neurons);
elseif pre_or_post == "pre"
    loadFolder = flag.pre_model_input;
    savingFileErr = flag.pre_err_model_output;
    savingFileCorr = flag.pre_corr_model_output;
...
[saveFolder, ~, ~] = fileparts(savingFileCorr);
...
parfor neu_num = 1:numNeurons % change to parfor for cluster
    ...
    md = load([loadFolder,neuron_address,'.mat'],'MyData'); % load MyData
    MyData = md.MyData;
    if sum(MyData.sorted.rewarded) > 10;
        corr_output = fit_single_neuron(neuron_address, loadFolder, "correct");
        corr_output.fitted = true;
    else
        corr_output = struct;
        corr_output.fitted = false;
    end
    corr_output.num_corr = sum(MyData.sorted.rewarded);

    if sum(~MyData.sorted.rewarded) > 10;
        err_output = fit_single_neuron(neuron_address, loadFolder, "error");
        err_output.fitted = true;
    else
        err_output = struct;
        err_output.fitted = false;
    end
    err_output.num_err = sum(~MyData.sorted.rewarded);
    
    all_results_corr{neu_num} = corr_output;
    all_results_err{neu_num} = err_output;
...
save(savingFileErr,'all_results_err','-v7');
save(savingFileCorr,'all_results_corr','-v7');
end

function output = fit_single_neuron(neuron_address, loadFolder, corr_flag)
    flag = config();
    ...
    neuronData.signalValue.rewarded = MyData.sorted.rewarded;

    if corr_flag == "correct"
        trial_include_idx = neuronData.signalValue.rewarded;
    elseif corr_flag == "error"
        trial_include_idx = ~neuronData.signalValue.rewarded;
    end
    ...
    % trial numbers for subset 
    tr_nums = repmat([1:size(fr_ctr,1)]', 1, size(fr_ctr,2));

    % construct variable to predict, y
    y = reshape(fr_ctr', [], 1);
    tr_nums = reshape(tr_nums', [], 1);
    ...
    % now we remove all points where any x is nan or y is nan or trial
    % number is not included
    x_nan = sum(isnan(x), 2) > 0;
    y_nan = isnan(y);
    exclude_idx = ~ismember(tr_nums,find(trial_include_idx));
    x(x_nan | y_nan | exclude_idx, :) = [];
    y(x_nan | y_nan | exclude_idx) = [];
    y = squeeze(y);
