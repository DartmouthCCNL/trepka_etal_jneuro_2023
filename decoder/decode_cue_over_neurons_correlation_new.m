    ...
    for pre_or_post = ["pre", "post"];
        for ls = ["long", "short"]
            ...
            for iters = 1:max_iters
                ...
                uids = unique(curr_id);
                uids = uids(randperm(length(uids)));
                ...
                cnts = 1;
                for sp_size = [1:5, 10:10:90]
                    X = mean(X_fr(:, 1:sp_size, 41:61), 3); % get fr during cue period

                    k = 10;
                    norm = true;

                    accuracy(cnts, iters) = getKFoldCorrLoss(X, y, k, norm);

                    shuff_y = y(randperm(length(y)));
                    shuff_accuracy(cnts, iters) = getKFoldCorrLoss(X, shuff_y, k, norm);
                    cnts = cnts+1;
                end
            end
    ...
    for pre_or_post = ["pre", "post"];
        ...
        for iters = 1:max_iters
            ...
            for jjj = 1:2
                ...
                cnts = 1;
                for sp_size = [1:5, 10:10:90]
                    X = mean(X_fr(:, 1:sp_size, 41:61), 3); % get fr during cue delay period period

                    k = 10;
                    norm = true;

                    if jjj == 1
                        long_accuracy(cnts, iters) = getKFoldCorrLoss(X, y, k, norm);
                    else
                        short_accuracy(cnts, iters) = getKFoldCorrLoss(X, y, k, norm);
                    end
                    cnts = cnts + 1;
                end
        ...
        for iters = 1:max_iters
            ...
            for jjj = 1:2
                ...
                for sp_size = [1:5, 10:10:90]
                    ...
                    if jjj == 1
                        pre_accuracy(cnts, iters) = getKFoldCorrLoss(X, y, k, norm);
                    else
                        post_accuracy(cnts, iters) = getKFoldCorrLoss(X, y, k, norm);
                    end
                    cnts = cnts + 1;
                end
            end
...
out_file = flag.decoder_output + "corr_accuracy_over_neurons_new.mat";
save(out_file, 'acc');
